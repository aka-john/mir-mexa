    ...
    public function editAction() 
    {
        ...
        if (CRM_IS_POST()) {
            ...
            if (count($validate) == 0) {
				...
				if($shop == TRUE AND $_POST['status'] != $change_status){
					
						$site_root = $_SERVER['DOCUMENT_ROOT'];
			
					
				//подсключаем api
				include_once($site_root."/assets/snippets/status/status_change.php");
					//echo 'статус был измене';

				}else{
					//echo 'статус не измене';
				}
            } 
        }
